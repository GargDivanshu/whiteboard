...
export const whiteboardFiles = pgTable("whiteboard_files", {
	...
	workspaceId: uuid("workspace_id").notNull().references(() => whiteboardWorkspaces.id, { onDelete: "cascade" } ),
	folderId: uuid("folder_id").notNull().references(() => whiteboardFolders.id, { onDelete: "cascade" } ),
});
...
export const whiteboardWorkspaces = pgTable("whiteboard_workspaces", {
	id: uuid("id").defaultRandom().primaryKey().notNull(),
	createdAt: timestamp("created_at", { mode: 'string' }),
